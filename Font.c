...
//#endif
 ptr->color = Qnil;
 ptr->cache = NULL;
 ret = Data_Wrap_Struct(klass, font_mark, font_free, ptr);
...
{
 VALUE name = Qnil;
 const double ptsizemul[4] = { 1.0, 1.0, 1.0, 0.75 };
// const struct Font *ptr = rb_font_data(self);
 struct Font *ptr = rb_font_data_mut(self);
//#if RGSS == 3
// Note: Font#size seems to be px under 96dpi. multiplying 3/4 to convert to pt.
 const double ptsize = as_double(ptr->size) * ptsizemul[rgssver];
...
{
// ((struct Font *)ptr)->cache = loadFont( StringValueCStr(name), (int)ptsize, ptr->bold, ptr->italic);
   ptr->cache = loadFont( StringValueCStr(name), (int)ptsize, ptr->bold, ptr->italic);
}
...
{
  rb_raise(rb_eTypeError, "can't convert %s into Font", rb_class2name(rb_obj_class(obj)));
}
...
void Init_Font(void)
{
 ...
 rb_define_private_method(rb_cFont, "initialize", rb_font_m_initialize, -1);
 rb_define_private_method(rb_cFont, "initialize_copy", rb_font_m_initialize_copy, 1);
