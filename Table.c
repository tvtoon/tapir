...
static struct Table *tablespa[4096] =
{
...
static bool rb_table_data_p(VALUE obj)
{
 if(TYPE(obj) != T_DATA) return false;
 return RDATA(obj)->dmark == (void(*)(void*))table_mark;
}

static struct Table *rb_table_data_mut(VALUE obj)
{
...
{
#ifdef __DEBUG__
  fprintf( stderr, "Reached maximum table count of 4096!\n" );
#endif
  rb_raise( rb_eRGSSError, "Reached maximum table count of 4096!\n" );
}
 else
{
  ptr = ALLOC(struct Table);
  ptr->dim = 0;
  ptr->xsize = 0;
  ptr->ysize = 0;
  ptr->zsize = 0;
  ...
  ptr->ownid = cminindex;
  tablespa[cminindex] = ptr;

  for ( cminindex++; cminindex < 4096; cminindex++ )
{
   if ( tablespa[cminindex] == 0 ) break;
}
...
// VALUE ret = table_alloc(rb_cTable);
 ...
 const char *s = 0;
 int i = 0;
 long s_len = cminindex;
// struct Table *ptr = rb_table_data_mut(ret);
 struct Table *ptr = 0;

 (void) klass;
 ret = table_alloc(rb_cTable);
...
{
  rb_raise(rb_eTypeError, "can't convert %s into Table", rb_class2name(rb_obj_class(obj)));
}
