...
static struct Bitmap *bitmapspa[MAXBMAP] =
{
...
static VALUE bitmap_alloc(VALUE klass)
{
 ...
 if ( cminindex == MAXBMAP )
{
  rb_raise( rb_eRGSSError, "Reached maximum bitmap count of %u!\n", MAXBMAP );
}
 else
{
...
// ptr->pixcol = rb_color_new(0.0, 0.0, 0.0, 0.0);
  ...
  for ( cminindex++; cminindex < MAXBMAP; cminindex++ )
{
